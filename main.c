...
//        4. Admin access for actually being able to use this in someone's PC?
//        Need to check this.
 ...
 *
 * @param parsed Command arguments (parsed[1] = source, parsed[2] = destination)
 * @return 1 if successful, 0 if failed
 */
int copy_file(char **parsed) {
  if (parsed[1] == NULL || parsed[2] == NULL) {
    printf("Error: cp requires source and destination file paths\n");
  ...
  while ((bytes_read = fread(buffer, 1, sizeof(buffer), source)) > 0) {
    if (fwrite(buffer, 1, bytes_read, destination) != bytes_read) {
      printf("Error: Write error while copying\n");
      fclose(source);
      fclose(destination);
      return 0;
    }
